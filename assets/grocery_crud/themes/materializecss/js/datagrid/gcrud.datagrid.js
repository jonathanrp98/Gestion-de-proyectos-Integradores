...
(function ($) {
    ...
    Datagrid.prototype.checkCachedData = function () {
        var perPageValue = $('.' + Datagrid.CLASS_PER_PAGE).closest("div." + Datagrid.CLASS_PER_PAGE).find("input.select-dropdown").val();
        ...
        if (perPageValue !== cachedDatra.per_page) {
            //$('.' + Datagrid.CLASS_PER_PAGE).val();
            $('.' + Datagrid.CLASS_PER_PAGE).closest("div." + Datagrid.CLASS_PER_PAGE).find("input.select-dropdown").val(cachedDatra.per_page);
            forceRefresh = true;
        }
    ...
    Datagrid.prototype.appendSearchClearButtons = function () {
        ...
        $('.grocery-crud-table input.searchable-input').each(function () {
            ...
            $(this).before('<i class="material-icons clear-search">clear</i>');

            $('.clear-search').click(function () {
                $(this).closest('th').find('.searchable-input').val('').trigger('keyup');
            });
        });
    };
    ...
    Datagrid.prototype.listenerPerPage = function () {
        ...
        $('.' + Datagrid.CLASS_PER_PAGE).change(function () {
        	$('select.per_page').material_select();
            $('.page-number-hidden').val('1');
            datagrid_object.SearchAndOrderingTrigger();
        });
    };
    ...
    Datagrid.prototype.SearchAndOrderingTrigger = function () {
        var order_by = $('.column-with-ordering.active:first').data('order-by'),
            ...
            per_page_value = $('.' + Datagrid.CLASS_PER_PAGE).closest("div." + Datagrid.CLASS_PER_PAGE).find("input.select-dropdown").val(),
            data_to_send;
        ...
        data_to_send = {
            ...
            per_page: $('.' + Datagrid.CLASS_PER_PAGE).closest("div." + Datagrid.CLASS_PER_PAGE).find("input.select-dropdown").val(),
    ...
    Datagrid.prototype.getMaxPaging = function () {
        var total_results = parseInt($('.current-total-results').html(), 10),
            per_page = parseInt($('.' + Datagrid.CLASS_PER_PAGE).closest("div." + Datagrid.CLASS_PER_PAGE).find("input.select-dropdown").val(), 10),
    ...
    Datagrid.prototype.listenerSelectRow = function () {
        ...
        $('.select-row').click(function () {
            if ($(this).is(':checked')) {
                $(this).closest('tr').addClass('red-text');
            } else {
                $(this).closest('tr').removeClass('red-text');
            }
    ...
    Datagrid.prototype.calculationsBeforeDelete = function (total_deleted_records) {
        var datagrid_object = this,
            ...
            my_current_per_page = parseInt($('.' + Datagrid.CLASS_PER_PAGE).closest("div." + Datagrid.CLASS_PER_PAGE).find("input.select-dropdown").val(), 10);
    ...
    Datagrid.prototype.listenerDeleteMultiple = function () {
        ...
        $('.delete-selected-button').click(function () {
            ...
            $('.delete-multiple-confirmation').modal('open');
            ...
            $('.delete-multiple-confirmation-button').click(function (event) {
                ...
                $.ajax({
                    ...
                    success: function (output) {
                        if (output.success) {
                        	Materialize.toast(output.success_message, 4000, 'green');
                        }

                        my_modal.modal('close');
                        datagrid_object.SearchAndOrderingTrigger();
                    }
                });
            });
