							...
							foreach($estado as $row2){
								if($row->id_estudiante == $row2->cod_estudiante){
									if($entrega == "primer_entrega"){
										$nota = $row2->N_primerEntrega;
									}else{
										$nota = $row2->N_finalEntrega;
									}
									$enable = "disabled";
									$opcion = "$nota<img width='40px' height='40px' src='".base_url()."src/icons/check-icon.png' class='img-fluid' alt='Responsive image'>";
								}
							}
							echo "<tr><td>$row->id_estudiante</td><td>$row->nombre</td><td><form action='".base_url()."index.php/asesor/enviar_calificacion' method='post'>$opcion</td><td><input type='text' name='id_estudiante' value='$row->id_estudiante' hidden>
								<input type='text' name='id_grupo' value='$row->grupo' hidden>
								<input type='text' name='estudiante' value='$row->id_estudiante' hidden>
								<input type='text' name='entrega' value='$entrega' hidden><button type='submit' $enable class='btn btn-primary'>
